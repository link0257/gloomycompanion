...
function create_scenario_list(scenarios, decklist, retobj, level)
{
    var radiolist = []
    for (var i = 0; i < scenarios.length; i++)
    {
        ...
        function update_retobj(decknames, level, e)
        {
            for (var i = 0; i < decknames.length; i++) {
                if (decknames[i] != "Boss") {
                    decknames[i] = decknames[i] + "-" + level;
                }
            }
        ...
        radio.onchange = update_retobj.bind(null, scenario.decks, level);
...
function init()
{
    ...
    var selected_decks = [];
    var selected_level = prompt('Enter difficulty level');
    create_deck_list(decks).map( function(checkbox) { decklist.appendChild(checkbox); } );
    create_scenario_list(SCENARIO_DEFINITIONS, decks, selected_decks, selected_level).map( function(radiobtn) { scenariolist.appendChild(radiobtn); } );
